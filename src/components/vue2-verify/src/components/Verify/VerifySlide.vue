<template>
  <div style="position: relative;" @mouseover="showImage = true" @mouseout="showImage = true">
    <!-- puzzle的情况 -->
    <div v-if="type === '2'" class="verify-img-out" v-show="showImage" @mouseover="showImage = true" @mouseout="showImage = true" :style="{display: mode === 'pop'?'none':undefined,
      ...
      <div class="verify-img-panel" :style="{width: setSize.imgWidth,
        ...
        <div class="verify-gap" :style="{'width': blockSize.width, 'height': blockSize.height, top: top + 'px', left: left + 'px'}"></div>
      </div>
    </div>
    ...
    <div class="verify-bar-area" :style="{width: setSize.barWidth,
      ...
      <div class="verify-left-bar" :style="{width: (leftBarWidth!==undefined)?leftBarWidth: barSize.height, height: barSize.height, 'border-color': leftBarBorderColor, transaction: transitionWidth}">
        ...
        <div class="verify-move-block" @touchstart="start" @mousedown="start" :style="{width: barSize.height, height: barSize.height, 'background-color': moveBlockBackgroundColor, left: moveBlockLeft, transition: transitionLeft}">
          ...
          <div v-if="type === '2'" class="verify-sub-block" :style="{'width': blockSize.width,
                                  ...
                                  'background-position-x': '-' + left + 'px'}" v-show="mode !== 'pop' && showImage"></div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    //随机出生点位
    randSet: function() {
      var verifyInfo = this.$store.state.verifyInfo;
      this.top = verifyInfo.point.top;
      this.left = verifyInfo.point.left;
    },
